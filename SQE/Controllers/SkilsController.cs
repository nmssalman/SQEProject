...
namespace SQE.Controllers
{
    ...
    public class SkilsController : ControllerBase
    {
        ...
        public readonly ILogger<SkilsController> _logger;
        public readonly IMapper _mapper;
        public readonly IAuthManager _authManager;

        public SkilsController(
            UserManager<ApiUser> userManager,
            //SignInManager<ApiUser> signInManager, 
            ILogger<SkilsController> logger,
            IMapper mapper,
        ...
        public async Task<IActionResult> getSkils()
        {
            ...
            try
            {
                var personalDetails = await _unitOfWork.Skils.GetAll(s=>s.ActiveStatus == true);
                var result = _mapper.Map<Skils>(personalDetails);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> createSkils([FromBody] CreateSkilsDOT createSkilsDOT)
        {
            ...
            try
            {
                var skils = _mapper.Map<Skils>(createSkilsDOT);
                await _unitOfWork.Skils.Insert(skils);
